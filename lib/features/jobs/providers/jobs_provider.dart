...
import '../../../core/constants/mock_jobs.dart';
...
final jobsProvider = FutureProvider<List<JobGroup>>((ref) async {
  ...
  List<JobModel> allJobs = [];
    try {
     ...
     final snapshot = await firestore.collection(FirestoreKeys.jobsCollection)
        .where('isActive', isEqualTo: true)
        .orderBy('postedAt', descending: true)
        .get();
        
      allJobs = snapshot.docs.map((doc) => JobModel.fromMap(doc.data(), doc.id)).toList();
  } catch (e) {
      debugPrint('Error Fetching Jobs, falling back to mock data: $e');
  }

  // Fallback to Mock Data if empty (for UI testing before Firebase is fully seeded)
  if (allJobs.isEmpty) {
    allJobs = MockJobs.defaultJobs;
  }
  ...
  final filteredJobs = allJobs.where((job) {
    ...
    final matchesContract = contractFilter == null || job.jobType == contractFilter;
    ...
    final matchesExperience = experienceFilter == null || job.level == experienceFilter;
